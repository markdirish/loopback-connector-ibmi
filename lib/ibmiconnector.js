...
class IBMiConnector extends SQLConnector {
  ...
  async executeSQL(sql, params, options, callback) {
    ...
    this.pool.query(sql, params, (error, result) => {
      let err = error;
      if (error && error.odbcErrors[0]) {
        err = new Error(error.odbcErrors[0].message);
        err.code = error.odbcErrors[0].code;
        err.state = error.odbcErrors[0].state;
      }
      callback(err, result);
    });
  }
  ...
  */
  ...
  connect(callback) {
    this.dataSource.driver.pool(this.connectionString, (error, pool) => {
      this.pool = pool;
      callback(error, pool);
    });
  }
